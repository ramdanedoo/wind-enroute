...
if run and route:
    ...
    st.write("")
    # Tabel (full width, enak di HP)
    cols_avail = ["ident", "fl", "wind_dir", "wind_spd", "headwind", "crosswind"]
    tbl = {"WPT": df["ident"], "FL": df["fl"].map(lambda x: f"FL{x:03d}"),
           "WIND": df["wind_dir"].astype(str).str.zfill(3)+"/"+df["wind_spd"].astype(str),
           "HW": df["headwind"], "XW": df["crosswind"]}
